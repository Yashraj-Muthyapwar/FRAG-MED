...
class FederatedConfig(BaseSettings):
    ...
    # Project Paths - Ensure we're at FRAG_MED root
    ...
    def print_paths(self):
        ...
        print("\n" + "="*80)
        print("📁 FEDERATED CONFIG PATHS")
        print("="*80)
        print(f"Config file location: {Path(__file__).resolve()}")
        print(f"PROJECT_ROOT: {self.PROJECT_ROOT}")
        print(f"FEDERATED_DIR: {self.FEDERATED_DIR}")
        print(f"MODELS_DIR: {self.MODELS_DIR}")
        print(f"EMBEDDING_MODEL_PATH: {self.EMBEDDING_MODEL_PATH}")
        print(f"\nModel exists? {self.EMBEDDING_MODEL_PATH.exists()}")
        if self.EMBEDDING_MODEL_PATH.exists():
            model_files = list(self.EMBEDDING_MODEL_PATH.rglob('*.bin')) + \
                         list(self.EMBEDDING_MODEL_PATH.rglob('*.safetensors'))
